...
const browser=await chromium.launch({headless:true});const p=await browser.newPage(),errors=[];p.on('pageerror',e=>errors.push(e.message));
...
for(let batch=0;batch<3;batch++){
  await p.evaluate(async()=>{
    const [engine,browser]=await Promise.all([import('/src/ui/engine-panel.ts'),import('/src/ui/browser.ts')]);
    for(let i=0;i<100;i++){engine.open();engine.close();browser.openBrowser('drum',i%5);browser.closeBrowser();}
  });
  await capture((batch+1)*100);console.log(JSON.stringify(snapshots.at(-1)));
}
const r={cycles:300,operations:1200,snapshots,errors,status:'PASS'};
