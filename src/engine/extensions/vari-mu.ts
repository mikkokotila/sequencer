    ...
    return {
        ...
        createUI(container: HTMLElement): void {
            ...
            makeSlider(container, 'DRIVE', state.drive, 0, 1, 0.01,
                v => formatPct(v), v => { state.drive = v; applyState(); window.SEQ.notifyStateChange(); });

            makeSlider(container, 'COMPRESS', state.compress, -50, 0, 1,
                v => Math.round(v) + ' dB', v => { state.compress = v; applyState(); window.SEQ.notifyStateChange(); });

            makeSlider(container, 'RATIO', state.ratio, 1, 20, 0.5,
                v => v.toFixed(1) + ':1', v => { state.ratio = v; applyState(); window.SEQ.notifyStateChange(); });

            makeSlider(container, 'KNEE', state.knee, 0, 40, 1,
                v => Math.round(v) + ' dB', v => { state.knee = v; applyState(); window.SEQ.notifyStateChange(); });

            makeSlider(container, 'SPEED', state.speed, 0, 3, 1,
                v => SPEED_PRESETS[Math.round(v)]?.label ?? '', v => { state.speed = Math.round(v); applyState(); window.SEQ.notifyStateChange(); });

            makeSlider(container, 'MIX', state.mix, 0, 1, 0.01,
                v => formatPct(v), v => { state.mix = v; applyState(); window.SEQ.notifyStateChange(); });

            makeSlider(container, 'OUTPUT', state.output, 0, 1, 0.01,
                v => formatPct(v), v => { state.output = v; applyState(); window.SEQ.notifyStateChange(); });
            ...
            fillEl.style.cssText = 'height:100%;width:0%;background:linear-gradient(90deg,#ff8c38,#ff3b5c);border-radius:3px;transition:width 0.06s linear;';
            grTrack.appendChild(fillEl);
            grSec.appendChild(grTrack);

            const valEl = document.createElement('div');
            valEl.style.cssText = 'font-size:10px;font-weight:700;color:#ff8c38;margin-top:4px;text-align:right;font-variant-numeric:tabular-nums;';
