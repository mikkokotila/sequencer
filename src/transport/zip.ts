/**
 * Minimal uncompressed (STORE) ZIP writer. Sufficient for bundling a handful
 * of WAV files where compression would not help (PCM audio is incompressible).
 */
...
export function buildStoreZip(entries: ZipEntry[]): Uint8Array {
  ...
  for (const entry of entries) {
    ...
    lv.setUint16(6, 0x0800, true); // bit flag: UTF-8 filename
    ...
    cv.setUint16(8, 0x0800, true); // bit flag
